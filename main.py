...
async def detect(request: ImageRequest):
    ...
    class_names = ['NAMA', 'NIK', 'Tempat Tanggal Lahir', 'ALAMAT', 'JENIS KELAMIN']

    # Dictionary to store the best detection for each class name
    best_detections = {class_name: {"conf": 0, "text": ""} for class_name in class_names}
    ...
    formatted_output = {class_name: best_detections[class_name]['text'] for class_name in class_names if best_detections[class_name]['conf'] > 0}
